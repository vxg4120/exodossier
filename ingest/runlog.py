...
Ported from the satellite platform's ingest/runlog.py. `polite_get` is the single choke point:
it checks the `ingest_run` ledger for a recent successful pull of the same source+endpoint and
skips (logging `skipped_fresh`) if one exists within `min_interval`. Otherwise it performs the
HTTP GET, tags the ingest_run row it created with the response so the caller can finish the run
once it knows how many rows it landed.

Callers are responsible for calling `finish_run(..., status="ok")` once they've parsed and landed
the payload — `polite_get` only owns the fetch, the skip check, and the error path.
"""
...
# User-Agent identifying the project (SPEC politeness rule: identify yourself on every pull).
USER_AGENT = "ExoDossier/0.1 (exoplanet identity-graph research project; polite; contact in repo)"
TIMEOUT_S = 180
...
def polite_get(
    ...
    **requests_kwargs,
    ...
    Returns the `requests.Response` on success, tagged with `.exo_run_id` (the ingest_run row this
    pull opened — the caller finishes it with the real row count) and `.exo_bytes` (bytes
    downloaded). Returns None if a fresh `ok` run already exists (a `skipped_fresh` row is logged
    for visibility, but no HTTP call is made). Raises and logs an `error` row on HTTP failure.
    """
    ...
    response.exo_run_id = run_id
    response.exo_bytes = len(response.content)
    return response
