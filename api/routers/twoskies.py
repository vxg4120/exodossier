"""GET /api/twoskies/* -- the Follow-up bridge: the exoplanet catalog meets the satellite catalog.

One view over two databases, framed from the exoplanet follow-up observer's chair. ExoDossier's own
identity graph (``exo``) knows where the interesting candidates sit on the sky; the satellite
identity graph (``oei``, read-only) knows where ~16k LEO objects are right now. This module joins
them: when you point a ground-based telescope at one of these candidates to confirm it, which
tracked satellites cross its line of sight during the window -- and who owns them.
...
The honest science (framed everywhere, never overclaimed):
  ...
  * Positions are ILLUSTRATIVE. The satellite element sets are up to ~1 week old, so propagated
    positions carry along-track error of many km (arcminutes-to-degrees on the sky). This answers
    "roughly which objects sweep this patch of sky", not operational, collision-grade questions.
  * Frames are approximate: TEME is treated as inertial (no precession/nutation), no refraction.

Three endpoints:
  * ``/targets``               -- ExoDossier's own exoplanet hosts (famous + conflict-flagged) with
                                  coordinates, read from the local ``exo`` graph.
  * ``/congestion-astronomy``  -- the megaconstellation-vs-astronomy panel over the ``oei`` catalog.
  * ``/passes``                -- THE bridge: SGP4-propagate the LEO catalog (``oei``) over a window
                                  and return satellites whose apparent (topocentric) position passes
                                  within ``sep_deg`` of the target sightline while the target is up.
...
from api.deps import get_db, get_oei_db
...
CAVEATS = [
    ...
    "follow-up -- the TFOP network and wide-field surveys that confirm TESS candidates from Earth.",
    "Positions are ILLUSTRATIVE, not operational. The satellite element sets are ~1 week old; "
    "propagated positions carry along-track error of many kilometres (arcminutes-to-degrees on the "
    "sky). Good for 'roughly which objects cross this sky patch', not collision-grade avoidance.",
...
# ---------------------------------------------------------------------------------------------
# Exoplanet-target queries (read-only, `exo` -- ExoDossier's OWN graph, so this is just get_db).
# Only the identity tables star + candidate are read; a host's conflict flag is the presence of an
# AMBIGUOUS-disposition candidate -- the resolved stand-in for "the source catalogs disagree about
# whether this is a real planet" (the ExoDossier conflict layer). Aggregation collapses each host to
# one sky point.
# ---------------------------------------------------------------------------------------------
_TARGETS_SQL = """
SELECT
    ...
    array_agg(c.candidate_id  ORDER BY c.canonical_name)   AS candidate_ids,
...
def _reduce_target(row: dict, category: str) -> dict:
    ...
    return {
        "candidate_id": ids[best_i],
        "candidate": names[best_i],
        "host": row["host"],
        "tic_id": str(row["tic_id"]) if row["tic_id"] is not None else None,
...
def targets(
    ...
    conflict_limit: Annotated[int, Query(ge=0, le=200)] = 24,
):
    """Curated exoplanet host stars with sky coordinates for the line-of-sight forecaster.

    Reads ExoDossier's OWN ``exo`` graph (star/candidate). Returns the famous benchmark systems plus
    a slice of conflict-flagged hosts (a candidate with an AMBIGUOUS/disputed disposition). One row
    per host (a single sky point); ``has_conflict`` flags the ones where the source catalogs
    disagree about whether it is even a planet.
    """
    with db.cursor() as cur:
        ...
        cur.execute(
            _TARGETS_SQL.format(
                where="s.star_id IN "
                "(SELECT star_id FROM candidate WHERE disposition = 'AMBIGUOUS') "
...
# ---------------------------------------------------------------------------------------------
# Congestion-vs-astronomy panel. All read-only aggregates over the satellite ``oei`` DB. The
# altitude x inclination density bins (the heatmap) plus the catalog-scale headline numbers + the
# operator concentration that drives the interference story. ``_CONGESTION_SQL`` is inlined here
# (ExoDossier has no satellite congestion router of its own).
# ---------------------------------------------------------------------------------------------
_CONGESTION_SQL = """
...
def congestion_astronomy(db: Annotated[psycopg.Connection, Depends(get_oei_db)]):
    ...
    Frames the megaconstellation-vs-astronomy interference story from the observer's chair. Reads
...
def _gmst_rad(jd_ut1: float) -> float:
    """Greenwich Mean Sidereal Time (radians) for the TEME->ECEF rotation. UT1 ~ UTC."""
    d = jd_ut1 - 2451545.0
    tc = d / 36525.0
...
def _get_catalog(db) -> tuple[SatrecArray, list[dict], np.ndarray, bool]:
    ...
    log.info(
        "twoskies: built %d satrecs from %d rows in %.2fs", len(meta), len(rows), time.time() - t0
    )
    with _CACHE_LOCK:
...
@router.get("/passes")
def passes(
    db: Annotated[psycopg.Connection, Depends(get_oei_db)],
    ra: Annotated[float, Query(ge=0.0, le=360.0, description="Target right ascension, deg (ICRS)")],
    dec: Annotated[float, Query(ge=-90.0, le=90.0, description="Target declination, deg (ICRS)")],
    datetime: Annotated[str | None, Query(description="UTC ISO-8601 start; default now")] = None,
    window_min: Annotated[int, Query(ge=5, le=180, description="window length, minutes")] = 60,
    sep_deg: Annotated[float, Query(ge=0.1, le=30.0, description="threshold, deg")] = 5.0,
    site: Annotated[str, Query(description="observatory preset key")] = DEFAULT_SITE,
    lat: Annotated[float | None, Query(ge=-90.0, le=90.0, description="override, deg")] = None,
    lon: Annotated[float | None, Query(ge=-180.0, le=180.0, description="override, degE")] = None,
    elev_km: Annotated[float | None, Query(ge=-0.5, le=6.0, description="override, km")] = None,
    step_sec: Annotated[int, Query(ge=15, le=120, description="time step, seconds")] = 30,
    min_alt_deg: Annotated[float, Query(ge=0.0, le=80.0, description="target-up gate, deg")] = 20.0,
    max_results: Annotated[int, Query(ge=1, le=2000, description="cap pass list")] = 300,
):
    """Satellites whose apparent position passes within ``sep_deg`` of the target line of sight.

    SGP4-propagates the LEO-payload catalog (``oei``) over ``window_min`` at ``step_sec`` from the
    given UTC time, converts each object to the observatory's topocentric frame, and reports
    every object whose closest approach to the target sightline falls under the threshold *while the
    target is above ``min_alt_deg``* (you observe -- and are contaminated -- only when it is up).
    ...
    if lat is not None and lon is not None:
        ...
        if site not in SITES:
            raise HTTPException(
                status_code=422, detail=f"unknown site {site!r}; presets: {', '.join(SITES)}"
            )
    ...
    # target altitude (deg) at each step, shape (nt,)
    tgt_alt = np.degrees(np.arcsin(np.clip(gx * up[0] + gy * up[1] + gz * up[2], -1.0, 1.0)))
    ...
    for i in hit_idx:
        ...
        passes_out.append({
            ...
            "time_utc": (
                start + dt.timedelta(seconds=step_sec * j)
    ...
    return {
        ...
        "window": {
            ...
            "end_utc": (
                start + dt.timedelta(minutes=window_min)
            ).isoformat().replace("+00:00", "Z"),
