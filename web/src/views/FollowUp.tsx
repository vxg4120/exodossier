...
export function FollowUp() {
  ...
  // A candidate handed over from the Target page: /followup?ra=&dec=&name= — pre-fills the forecaster
  // so checking follow-up passes is one click from vetting a candidate.
  const prefill: TwoSkyTarget | null = useMemo(() => {
    const ra = Number(params.get("ra"));
    const dec = Number(params.get("dec"));
    if (!Number.isFinite(ra) || !Number.isFinite(dec) || !params.get("ra") || !params.get("dec")) {
      return null;
    }
    return {
      candidate_id: null,
      candidate: params.get("name") ?? "Selected candidate",
      host: params.get("name") ?? "Selected candidate",
      tic_id: null,
      ra_deg: ra,
      dec_deg: dec,
      disposition: "from candidate page",
      has_conflict: params.get("conflict") === "1",
      n_candidates: 1,
      category: "custom",
    };
  }, [params]);
  ...
  const targetList = useMemo(
    () => (prefill ? [prefill, ...(targets.data?.targets ?? [])] : targets.data?.targets ?? []),
    [prefill, targets.data],
  );
  ...
  // When a candidate is handed over, jump the selection to it (index 0) once targets have loaded.
  useEffect(() => {
    if (prefill) setSelected(0);
  }, [prefill]);
  ...
  return (
    <div className="view fadein">
      <header className="vhead">
        <div>
          <h1 className="vhead__title">Follow-up · Two Skies</h1>
          <p className="vhead__desc">
            You confirm a TESS candidate from the ground — and you are no longer alone under the sky.
            This bridge joins ExoDossier&rsquo;s exoplanet hosts to the live satellite catalog and asks
            the follow-up observer&rsquo;s question: when you point a telescope at one of these
            candidates to vet it, which satellites cross its line of sight during your window — and who
            owns them.
          </p>
        </div>
      </header>
        ...
        <div className="ts-caveat">
          ...
          <ul>
            ...
            <li>
              ...
              propagated positions drift by arcminutes-to-degrees. Good for &ldquo;roughly which
              objects cross this patch of sky,&rdquo; not collision-grade.
            </li>
            <li>
            ...
            <>
              <div className="ts-form">
                <div className="ts-field ts-field--grow">
                  ...
                  >
                    {prefill ? (
                      <optgroup label="From candidate page">
                        <option value={0}>{optLabel(prefill)}</option>
                      </optgroup>
                    ) : null}
              ...
              {current ? (
                <p className="hint" style={{ marginTop: 8 }}>
                  ...
                  {current.candidate_id != null ? (
                    <>
                      {" "}
                      ·{" "}
                      <Link to={`/target/${current.candidate_id}`} className="hint">
                        view dossier ›
                      </Link>
                    </>
                  ) : null}
  ...
  return (
    <>
      ...
      {data.n_found === 0 ? (
        ...
        <>
          <DataTable columns={PASS_COLUMNS} rows={data.passes} rowKey={(r) => r.norad} />
          {data.truncated ? (
            <p className="hint" style={{ marginTop: 8 }}>
